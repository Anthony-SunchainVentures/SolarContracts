...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Errors {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual {
        to = address(0); // this will stop transfer , to make it SBT
        if (to == address(0)) {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal {
        to = address(0); // it will stop transfer, to make it SBT
        if (to == address(0)) {
            revert ERC721InvalidReceiver(address(0));
        }
        address previousOwner = _update(to, tokenId, address(0));
...
contract SolarSBT is ERC721, Ownable {
    ...
    function changeMaxSoulLimit(uint _newLimit) public onlyOwner returns(bool){
        maxSoulLimit = _newLimit;
        return true;
    }
    function changeExpiryInSecond(uint _seconds) public onlyOwner returns(bool){
